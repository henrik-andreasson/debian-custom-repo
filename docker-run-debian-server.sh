...
docker run --rm -it \
    ...
    --network repo \
    custom_debian_repo \
    -f /opt/custom-debian-repo/config/debian-server-packages.txt  \
    -s /opt/custom-debian-repo/config/debian.sources.list \
    -r /opt/iso-and-repo/repos/debianserver/20260304/ \
    -d -n
